 ...
 * 配置config.js  执行顺序  resize.js - index.js - mergeBounds.js
 */
...
async function resizeOrgImg(zoomRange) {
  ...
  console.log('放大到baseZoom', `${originalWidth} -> ${imgWidthInBaseZoom}`);
  ...
  for (let currentZoom = endZoom; currentZoom >= startZoom; currentZoom--) {
    ...
    console.log(currentZoom, `${scaledWidth} * ${scaledHeight}`);
