...
async function createTiles(breakpointZoomRange, breakpointStart) {
  ...
  for (let currentZoom = endZoom; currentZoom >= startZoom; currentZoom--) {
    ...
    for (let x = breakX; x <= Math.floor(xRange[1]); x++) {
      let ii = 0
      for (let y = breakY; y <= Math.floor(yRange[1]); y++) {
        ...
        let consoleText = `${currentZoom}级 - ${progress} ( ${currentTiles} / ${totalTiles}片 )  ${xyRangeProgress}  ${fileName}`;
